...
namespace Elysium.Combat
{
    ...
    {
        public static void DealDamage(IDamageDealer _applier, IElement _applierElement, IDamageable _receiver, int _base, float[] _additiveMultipliers, float[] _multiplicativeMultipliers, bool canCrit = false)
        {
            int damage = CalculateDamage(_applierElement, _receiver, _base, _additiveMultipliers, _multiplicativeMultipliers);
        ...
        public static bool IsCriticalHit(IDamageDealer _damageDealer, int _before, out int _after)
        {
            ...
            if (hasCrit && _damageDealer != null) { _damageDealer.OnCriticalHitCallback(); }
            _after = hasCrit ? Mathf.CeilToInt(_before * critDamage) : _before;
            return hasCrit;
        }

        public static int CalculateDamage(IElement _applierElement, IDamageable _receiver, int _base, float[] _additiveMultipliers, float[] _multiplicativeMultipliers)
        {
            ...
            int damage = Mathf.CeilToInt(_base * final);
